...
import { Button } from '@admin-basic/Button';
import { Div } from '@admin-basic/Div';
import { Icon } from '@admin-basic/Icon';
import { Select } from '@admin-basic/Select';
...
const SortableBgThumb: React.FC<SortableBgThumbProps> = ({
  ...
  return (
    <Div
      ref={setNodeRef}
      style={style}
      className="moabom-home-bg-thumb relative w-full min-w-0"
    >
      <Div className="overflow-hidden rounded-lg border border-gray-200 bg-gray-100 dark:border-gray-600 dark:bg-gray-700">
        <Div className="moabom-home-bg-thumb-preview bg-gray-200 dark:bg-gray-600">
          {src ? (
            <img src={src} alt="" className="h-full w-full object-cover" loading="lazy" />
          ) : (
            <Div className="flex h-full items-center justify-center text-xs text-gray-500 dark:text-gray-400">…</Div>
          )}
        </Div>
      </Div>
      <button
      ...
      {/* 모드 선택 */}
      <Select
        className="mt-1 w-full rounded border border-gray-300 bg-white px-1 py-0.5 text-xs text-gray-800 disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
        value={mode}
        disabled={!!readOnly}
        aria-label="배경 노출 모드"
      ...
      <Div ref={paletteRootRef} className="relative mt-1">
        <button
          ...
          className="flex w-full items-center justify-center gap-1 rounded border border-gray-300 bg-white px-1 py-1 text-xs text-gray-700 disabled:opacity-60 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100"
        >
          {pointColor ? (
            <Span
        ...
        {paletteOpen && (
          <Div
            role="listbox"
            className="absolute left-0 top-full z-50 mt-1 w-full rounded-lg border border-gray-200 bg-white p-2 shadow-lg dark:border-gray-600 dark:bg-gray-800"
          >
            {/* 없음 버튼 */}
            <button
              type="button"
              ...
              className={`mb-1 flex w-full items-center justify-center gap-1 rounded px-1 py-1 text-xs ${
                !pointColor
                ...
                return (
                  ...
                  >
                    {selected ? (
                      <Icon
                        name="check"
                        className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-xs text-white drop-shadow-sm"
                        aria-hidden
                      />
                    ) : null}
                  </button>
      ...
      <button
        type="button"
        className={`mt-1 flex w-full items-center justify-center gap-1 rounded border border-dashed border-gray-300 py-0.5 text-xs text-gray-500 dark:border-gray-600 ${readOnly ? 'invisible' : ''}`}
        {...attributes}
        {...listeners}
      >
        <Icon name="grip-vertical" className="text-xs" />
        <Span>이동</Span>
      </button>
    </Div>
  );
    ...
    <Div className={className}>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={items.map(i => i.id)} strategy={verticalListSortingStrategy}>
          <Div className="moabom-home-bg-grid">
            {items.map(item => (
              <SortableBgThumb
                key={item.id}
                item={item}
