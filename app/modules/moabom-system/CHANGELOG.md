...
- 알림센터 사용자 옵션명을 `알림센터 기록·배지 표시`로 간결하게 정리했습니다.
- 과거 시스템 옵션 저장 순서가 현재 기본값과 다를 때 날씨 효과가 중복 생성되지 않도록 옵션 id 기준 병합으로 수정했습니다.

## [0.8.45] - 2026-08-06

### Changed

- 동일 이벤트의 30초 중복 차단은 유지하고, 정상적인 다중 답글 알림이 누락되지 않도록 수신자·유형·채널별 분당 상한을 100건으로 조정했습니다.

## [0.8.44] - 2026-08-06

### Added

- 일반 알림 기록 선택, 필수 서비스 알림 보존, 중복·폭주 차단 정책을 추가했습니다.
- 읽음 30일·미읽음 90일·발송 이력 90일 및 사용자당 최신 500건 순환 보관을 적용했습니다.
- 사용자별 순환 삭제와 보관기간 정리용 알림 복합 인덱스를 추가했습니다.

## [0.8.43] - 2026-08-06

### Added

- 사용자 시스템 설정에 푸시 알림 수신 옵션과 서버 검증 계약을 추가했습니다.

## [0.8.42] - 2026-08-06

### Added

- Cloud Tasks 알림 처리 경로를 사용자 데이터 없이 배포 시점에 검증할 수 있도록 queue probe를 추가했습니다.
- queue HTTP task가 장기 worker daemon 대신 요청당 DB job 하나만 처리해 scoped request 초기화와 500을 방지합니다.

## [0.8.41] - 2026-08-05

### Changed

- 홈 HTML과 shell-boot가 동일한 config/home 스냅샷을 공유해 콜드스타트와 캐시 갱신 시 레이아웃 조립을 반복하지 않도록 개선했습니다.

## [0.8.40] - 2026-08-05

### Changed

- critical 사용자 shell-state에서 생성앱 라이브러리 집계를 제외해 전용 library API와의 로그인 직후 중복 platform DB 조회를 제거했습니다.
- 장애 복구 realtime-state가 요청된 notifications·chat·presence 도메인만 집계하고, template routes 캐시 키에 tenant scope를 포함하도록 변경했습니다.

## [0.8.39] - 2026-08-05

### Changed

- 로그인 메인 패널의 임계 사용자 상태를 먼저 응답하고 알림·접속 상태 조회는 후속 단계로 분리해 계정별 앱 배치 대기 시간을 줄였습니다.

## [0.8.38] - 2026-08-04

### Changed

- 네트워크 부트: `shell-boot` 응답을 인스턴스별 캐시에 더해 GCS 공유 스냅샷으로 read-through 캐싱합니다. 콜드스타트·오토스케일로 새 인스턴스가 떠도 재구성 없이 부트 임계 응답을 즉시 제공하며, 설정·모듈·revision 변경 시 자동 무효화됩니다(다중 인스턴스 GCS 환경에서만 활성).

## [0.8.37] - 2026-08-03

### Changed

- 네트워크 부트: shell-boot·Blade `shellCritical` 조립 시 `getRoutesDataWithModules` 이중 호출 제거 — `buildConfigHomeOnly()`로 config/home만 계산하고 routes는 `shell_routes` 1회만 머지합니다.
