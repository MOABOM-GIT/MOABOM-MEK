<?php

return [
    'apps' => [
        'ai' => [
            'generate_success' => 'AI 앱 HTML을 생성했습니다.',
            'stream_started' => 'AI 앱 HTML 생성을 시작했습니다.',
            'continue_default_prompt' => '중단된 부분부터 이어서 나머지 HTML 코드를 완성해주세요.',
            'session_fetch_success' => 'AI 생성 세션을 조회했습니다.',
            'session_not_found' => 'AI 생성 세션을 찾을 수 없습니다.',
            'session_resume_available' => '이어서 생성할 수 있는 세션이 있습니다.',
            'session_cancel_success' => 'AI 생성을 중지하고 세션을 정리했습니다.',
            'queue_waiting' => '현재 AI 생성 요청이 많아 대기열 :position번째입니다. 예상 대기 약 :minutes분입니다.',
            'queue_ready' => '대기가 끝났습니다. 곧 생성을 시작합니다.',
            'queue_cancel_success' => '대기열에서 요청을 취소했습니다.',
            'queue_ticket_required' => '대기열 티켓이 필요합니다.',
            'queue_ticket_not_found' => '대기열 티켓을 찾을 수 없거나 만료되었습니다.',
            'queue_full' => '대기열이 가득 찼습니다. 잠시 후 다시 시도해주세요.',
            'queue_denied' => '지금은 AI 생성을 시작할 수 없습니다. 잠시 후 다시 시도해주세요.',
            'notice' => [
                'default' => 'AI provider 설정 전까지 표시되는 기본 미리보기입니다.',
                'anthropic_no_key' => 'Anthropic API 키가 설정되지 않아 미리보기 HTML을 생성했습니다.',
                'anthropic_failed' => 'Anthropic API 호출에 실패해 미리보기 HTML을 생성했습니다.',
                'openai_no_key' => 'OpenAI API 키가 설정되지 않아 미리보기 HTML을 생성했습니다.',
                'openai_failed' => 'AI 생성 API 호출에 실패해 미리보기 HTML을 생성했습니다.',
                'google_no_key' => 'Google AI API 키가 설정되지 않아 미리보기 HTML을 생성했습니다.',
                'google_failed' => 'Google AI API 호출에 실패해 미리보기 HTML을 생성했습니다.',
            ],
        ],
        'website_link' => [
            'resolve_success' => '웹사이트 정보를 확인했습니다.',
            'resolve_failed' => '웹사이트 주소를 확인하지 못했습니다.',
            'url_required' => '웹사이트 주소를 입력해주세요.',
            'url_invalid' => '올바른 웹사이트 주소가 아닙니다.',
            'url_blocked' => '연결할 수 없는 주소입니다.',
        ],
        'generated' => [
            'fetch_success' => '마이앱 목록을 조회했습니다.',
            'show_success' => '마이앱을 조회했습니다.',
            'not_found' => '마이앱을 찾을 수 없습니다.',
            'save_success' => '마이앱을 저장했습니다.',
            'update_success' => '마이앱을 수정했습니다.',
            'share_success' => '공개 앱 등록 상태를 변경했습니다.',
            'delete_success' => '마이앱을 삭제했습니다.',
            'owner_unknown' => '알 수 없는 제작자',
            'untitled_app' => '제목 없는 앱',
            'revisions_fetch_success' => '앱 버전 목록을 조회했습니다.',
            'revision_show_success' => '앱 버전을 조회했습니다.',
            'revision_not_found' => '앱 버전을 찾을 수 없습니다.',
            'revision_restore_success' => '선택한 버전으로 복원했습니다.',
            'data_tables_success' => '앱 데이터 테이블 목록을 조회했습니다.',
            'data_rows_success' => '앱 데이터 행을 조회했습니다.',
            'data_row_delete_success' => '앱 데이터 행을 삭제했습니다.',
            'data_export_success' => '앱 데이터를 내보냈습니다.',
            'data_not_hosted' => '앱 서버 저장(Hosted) 앱만 데이터 콘솔을 사용할 수 있습니다.',
            'user_public_list_success' => '마이앱 목록을 조회했습니다.',
            'user_frequent_apps_success' => '최근 실행 앱 목록을 조회했습니다.',
            'security' => [
                'parent_shell_escape' => '부모 창(셸) 접근 코드는 보안상 허용되지 않습니다.',
                'cookie_exfiltration' => '쿠키 탈취·외부 전송 패턴은 허용되지 않습니다.',
                'javascript_protocol' => 'javascript: URL은 허용되지 않습니다.',
                'insecure_remote_script' => 'HTTP 외부 스크립트 주소는 허용되지 않습니다. HTTPS CDN 을 사용해주세요.',
                'obfuscated_eval' => '난독화된 eval 실행 패턴은 허용되지 않습니다.',
                'meta_refresh' => 'meta refresh 리다이렉트는 허용되지 않습니다.',
                'data_html_iframe' => 'data: URL iframe은 허용되지 않습니다.',
                'save_blocked' => '보안 검사에 실패해 저장할 수 없습니다. 아래 안내를 확인해주세요.',
            ],
        ],
        'seo' => [
            'list_success' => '앱 SEO 목록을 조회했습니다.',
            'show_success' => '앱 SEO 정보를 조회했습니다.',
            'not_found' => '공개된 앱을 찾을 수 없습니다.',
            'index_title' => '앱 디렉터리',
            'index_description' => '모아봄의 기본 제공 앱과 AI로 만든 공개 웹앱을 한눈에 둘러보고 바로 실행해 보세요.',
            'open_app' => '앱 열기',
            'open_live' => '실행 화면 보기',
            'directory_link' => '전체 앱 디렉터리',
            'category_basic' => '기본 제공 앱',
            'category_user' => '공개 앱',
        ],
        'community' => [
            'summary_success' => '앱 리뷰 요약을 조회했습니다.',
            'list_success' => '앱 리뷰 목록을 조회했습니다.',
            'show_success' => '앱 리뷰 글을 조회했습니다.',
            'create_success' => '리뷰를 등록했습니다.',
            'update_success' => '리뷰를 수정했습니다.',
            'delete_success' => '리뷰를 삭제했습니다.',
            'forbidden' => '앱 리뷰에 접근할 권한이 없습니다.',
            'review_exists' => '이미 이 앱에 리뷰를 작성했습니다.',
            'author_tenant_unresolved' => '업체(테넌트) 정보를 확인할 수 없어 리뷰를 저장할 수 없습니다. 업체 사이트에서 다시 시도해주세요.',
        ],
        'admin' => [
            'generated' => [
                'list_success' => '마이앱 목록을 조회했습니다.',
                'show_success' => '마이앱을 조회했습니다.',
                'visibility_success' => '마이앱 공개 범위를 변경했습니다.',
                'delete_success' => '마이앱을 완전히 삭제했습니다.',
            ],
            'community' => [
                'list_success' => '앱 리뷰 목록을 조회했습니다.',
                'show_success' => '앱 리뷰 글을 조회했습니다.',
                'status_success' => '앱 리뷰 상태를 변경했습니다.',
                'delete_success' => '앱 리뷰 글을 삭제했습니다.',
            ],
        ],
    ],
];
