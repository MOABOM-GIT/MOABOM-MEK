...
- (DataGate) `auto_fetch: false` 데이터소스를 progressiveDataInit에서 제외 — deferred fetch 키가 `undefined`로 남아 `blur_until_loaded: true`가 영구 블러되던 회귀 방지 (TemplateApp, websocket 제외와 동일 계약)
