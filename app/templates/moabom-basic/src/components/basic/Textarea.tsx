...
import { moaFieldTextareaClass } from '../../theme/moabomFieldSurface';
...
export const Textarea: React.FC<TextareaProps> = ({
  ...
  /** `Select` 네이티브 경로와 동일: 비어 있으면 재사용 표면, `bg-*` 또는 이미 `moaField*` 문자열이면 그대로, 그 외는 표면 + 유틸 병합 */
  const resolvedClassName = useMemo(() => {
    const trimmed = (className ?? '').trim();
    if (!trimmed) {
      return moaFieldTextareaClass('medium');
    }
    ...
    return moaFieldTextareaClass('medium', trimmed);
  }, [className]);
