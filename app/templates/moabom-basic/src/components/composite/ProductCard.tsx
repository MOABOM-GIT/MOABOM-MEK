        ...
        {labels.length > 0 && (
          <Div className="absolute bottom-2 right-2 flex gap-1 flex-wrap justify-end">
            {labels.map((label, idx) => (
              <Span
                key={idx}
                className="text-xs text-white font-bold px-1.5 py-0.5 rounded"
      ...
      <Div className="p-4">
        ...
        <Div className="flex items-center gap-1.5 flex-wrap">
          {product.primary_category && (
            <Span className="inline-block text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded">
              {product.primary_category}
            </Span>
          )}
          {product.brand_name && (
        ...
        {nameHighlighted ? (
          <Div
            className="mt-1 font-bold text-primary line-clamp-1 [&_mark]:bg-yellow-200 [&_mark]:dark:bg-yellow-500/40 [&_mark]:px-0.5 [&_mark]:rounded"
            dangerouslySetInnerHTML={{ __html: sanitizeHighlight(nameHighlighted) }}
          />
        ) : (
          <H3 className="mt-1 font-bold text-primary line-clamp-1">
            {displayName}
          </H3>
        )}
        ...
        {descriptionHtml && (
          <Div
            className="mt-1 text-sm text-muted line-clamp-2 [&_mark]:bg-yellow-200 [&_mark]:dark:bg-yellow-500/40 [&_mark]:px-0.5 [&_mark]:rounded"
        ...
        <Div className="mt-1.5 flex items-center gap-1">
          {getStarTypes(product.rating_avg ?? 0).map((type, idx) => (
            <Span
              key={idx}
              className={`text-xs ${type === 'empty' ? 'text-faint' : 'text-yellow-400'}`}
            >
