...
const SubText: React.FC<{ text?: string; title?: string }> = ({ text, title }) => {
  if (!text) return null;
  return (
    <Span className="text-sm text-muted" title={title}>
      {text}
    </Span>
  );
};
...
export const UserInfo: React.FC<UserInfoProps> = ({
    ...
    return (
      <Div className={containerClass}>
        <Div className="flex items-center gap-1.5 text-faint">
          ...
          <Span className="text-xs px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-muted rounded">
  ...
  const defaultColorClass = hasCustomColor ? '' : 'text-primary';
  const nameButtonClass = (showDropdown || clickable)
    ? `${defaultColorClass} hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer font-bold`.trim()
    : `${defaultColorClass} font-bold`.trim();
