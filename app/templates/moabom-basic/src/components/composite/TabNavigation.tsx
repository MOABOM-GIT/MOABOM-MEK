...
export const TabNavigation: React.FC<TabNavigationProps> = ({
  ...
  const getTabClasses = (tab: Tab) => {
    ...
    const baseClasses = isMobile && mobileGrid
      ? 'flex flex-col items-center justify-center gap-3 pt-4 pb-2 font-bold text-sm transition-all'
      : 'flex items-center gap-2 px-3 py-2 font-bold text-sm transition-all shrink-0 whitespace-nowrap';

    if (tab.disabled) {
      return `${baseClasses} opacity-50 cursor-not-allowed text-faint`;
    }

    // 모바일 그리드 스타일
    if (isMobile && mobileGrid) {
      return isActive
        ? `${baseClasses} text-primary bg-gray-100 dark:bg-gray-700 rounded-lg`
        : `${baseClasses} text-secondary hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg`;
    }
    ...
    switch (variant) {
      case 'pills':
        return isActive
          ...
          : `${baseClasses} text-secondary hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg`;

      case 'underline':
        return isActive
          ? `${baseClasses} text-primary border-b-2 border-gray-800 dark:border-white`
          : `${baseClasses} text-secondary hover:text-primary dark:hover:text-primary`
      ...
      default:
        return isActive
          ...
          : `${baseClasses} text-secondary hover:text-primary dark:hover:text-primary hover:bg-gray-50 dark:hover:bg-gray-700 border-b-2 border-transparent`;
    }
  };
