...
function sanitizeClassName(className: string, name: string): string {
    ...
    // Grid: `grid-cols-N` 만 축소 (`@sm:grid-cols-N` 등 반응형 토큰은 건드리지 않음)
    if (name === 'Grid' || /\bgrid-cols-\d/.test(sanitized)) {
        sanitized = sanitized
            .split(/\s+/)
            .map(tok => {
                const m = tok.match(/^grid-cols-(\d+)$/);
                if (!m) return tok;
                const num = parseInt(m[1], 10);
                return num > 4 ? `grid-cols-${Math.ceil(num / 2)}` : tok;
            })
            .join(' ')
            .replace(/\s{2,}/g, ' ')
            .trim();
...
function renderChildren(
    ...
    return filtered.map((child, i) =>
        renderSkeletonNode(child, animClass, iterCount, i, layoutWidth)
    );
}

/**
 * 현재 레이아웃 폭(창 본문 `moa-app-window-viewport` 측정값)에 맞는 className 결정
 *
 * responsive 속성이 있으면 Tailwind 기본 브레이크포인트와 동일한 수치로 선택.
 * - desktop (lg, 1024px+): responsive.desktop.props.className
 * - tablet (md, 768px+): responsive.tablet.props.className
 * - 기본: props.className
 */
function resolveClassName(component: LayoutComponent, layoutWidth: number): string {
 ...
 *
 * 레이아웃 JSON의 className에 `hidden @lg:grid`, `hidden lg:grid`, `@lg:hidden` 등 반응형 패턴이 포함됨.
 * Tailwind은 빌드 시 CSS로 처리하지만, 스켈레톤은 런타임에서 JS로 해석해야 함.
 *
 * 동작:
 * 1. display 관련 + `@?`(sm|md|lg|xl|2xl): 접두사 토큰 추출
 * 2. base → sm → md → lg → xl → 2xl 순으로 **레이아웃 폭**에 맞는 display 결정 (수치는 Tailwind 기본과 동일)
 * 3. hidden이면 컴포넌트 숨김, 아니면 해석된 display + 나머지 클래스 반환
 */
function resolveResponsiveDisplay(className: string, layoutWidth: number): { visible: boolean; className: string } {
    const width = layoutWidth;
    ...
    for (const token of tokens) {
        // 반응형 접두사 + display 클래스: "@lg:grid", "lg:grid", "sm:flex", "md:hidden"
        ...
        if (responsiveMatch) {
            ...
            // 반응형 접두사지만 display가 아닌 클래스 (@lg:gap-4 등) → 컨테이너 폭 기준 포함 (접두사 유지)
...
function renderSkeletonNode(
    ...
    // 1. responsive 속성에서 컨테이너(창 본문) 폭에 맞는 className 선택
    const rawClassName = resolveClassName(component, layoutWidth);
    ...
    const responsive = resolveResponsiveDisplay(exprResolved, layoutWidth);
    ...
    if (LAYOUT_CONTAINERS.has(name)) {
        ...
        const renderedChildren = renderChildren(children, animClass, iterCount, layoutWidth);
    ...
    if (children && children.length > 0) {
        return (
            <div key={index} className={sanitizedLeafClass}>
                {renderChildren(children, animClass, iterCount, layoutWidth)}
            </div>
        );
    }
...
export const PageSkeleton: React.FC<PageSkeletonProps> = ({ components, options }) => {
    ...
    React.useLayoutEffect(() => {
        ...
        const readWidth = () =>
            el.clientWidth > 0 ? el.clientWidth : (typeof window !== 'undefined' ? window.innerWidth : 1024);
        const ro = new ResizeObserver(() => {
            setLayoutWidth(readWidth());
        });
        ro.observe(el);
        setLayoutWidth(readWidth());
    ...
    return (
        <div
            ...
            aria-label="Loading..."
        >
            {renderChildren(components, animClass, options.iteration_count, layoutWidth)}
        </div>
    );
};
