    ...
    >
      ...
      >
        ...
        {(preview || existingImageUrl) ? (
          ...
          /* 파일 아이콘 */
          <I className={`fa-solid ${getFileIcon(file.mime_type)} text-4xl text-faint`} />
        )}
        ...
        <Div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-start justify-end p-1 gap-1 opacity-0 group-hover:opacity-100">
          ...
          <Button
            ...
            {...attributes}
            {...listeners}
            className="p-1.5 bg-white/90 dark:bg-gray-800/90 rounded text-muted hover:bg-white cursor-grab active:cursor-grabbing"
      ...
      >
        <P className="text-xs font-bold text-primary truncate" title={file.original_filename}>
