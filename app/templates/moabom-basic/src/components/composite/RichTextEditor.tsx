    ...
    <Div className={`border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden ${className}`}>
      ...
      <Div className="flex flex-wrap items-center gap-1 p-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        ...
        <Div className="flex items-center gap-1 pr-2 border-r border-gray-200 dark:border-gray-700">
          {toolbarButtons.map((btn) => (
            <Button
              ...
              disabled={disabled}
              className="w-8 h-8 flex items-center justify-center text-sm font-bold text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded disabled:opacity-50"
              title={btn.title}
            >
              {btn.icon}
            </Button>
        ...
        <Div className="flex items-center gap-1 pr-2 border-r border-gray-200 dark:border-gray-700">
          {headingButtons.map((btn) => (
            <Button
              ...
              onClick={() => applyFormat(btn.format)}
              disabled={disabled}
              className="px-2 h-8 text-sm font-bold text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded disabled:opacity-50"
            >
              {btn.label}
            </Button>
          ))}
        ...
        <Div className="flex items-center gap-1 pr-2 border-r border-gray-200 dark:border-gray-700">
          {listButtons.map((btn) => (
            <Button
              ...
              onClick={() => applyFormat(btn.format)}
              disabled={disabled}
              className="w-8 h-8 flex items-center justify-center text-sm text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded disabled:opacity-50"
              title={btn.title}
            >
              {btn.icon}
            </Button>
        ...
        <Div className="flex items-center gap-1 pr-2 border-r border-gray-200 dark:border-gray-700">
          <Button
            ...
            disabled={disabled}
            className="w-8 h-8 flex items-center justify-center text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded disabled:opacity-50"
            title={t('editor.toolbar.link')}
          >
            🔗
          </Button>
          <Label className="w-8 h-8 flex items-center justify-center text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded cursor-pointer">
            🖼
            <Input
              type="file"
              accept="image/*"
        ...
        <Button
          ...
          disabled={disabled}
          className={`px-2 h-8 text-sm text-secondary hover:bg-gray-200 dark:hover:bg-gray-700 rounded disabled:opacity-50 ${
      ...
      {showCodeView ? (
        ...
        <Div
          ...
          className="p-4 bg-white dark:bg-gray-900 text-primary focus:outline-none prose dark:prose-invert max-w-none"
          style={{ minHeight }}
        ...
        <Div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <Div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96">
            ...
            <Input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-primary mb-4"
            />
            <Div className="flex justify-end gap-2">
              <Button
                ...
                className="px-4 py-2 text-secondary hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"
              >
                {t('common.cancel')}
              </Button>
              <Button
