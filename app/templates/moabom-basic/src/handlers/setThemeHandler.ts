 ...
 *
 * moabom-admin_basic 템플릿과 동일한 키를 사용하여 테마 설정을 공유합니다.
 *
 * C8 단일화: 셸 테마 적용은 moabom-system 단일 소스(`applyMoabomSystemThemeMode`)로
 * 위임한다. 과거에는 이 핸들러가 `data-theme` + `.dark` 를 독립적으로 토글해
 * `applyMoabomSystemAppearance`(`data-moa-theme` + `.dark`)와 `.dark` 클래스를 두고
 * 경합했다(셸 CSS 는 `data-moa-theme` 만 사용하므로 `data-theme` 단독 토글은 desync).
 * 이제 두 경로가 동일한 SSOT 함수를 호출하므로 `.dark` 와 `data-moa-theme` 가 항상 일치한다.
 */

import { applyMoabomSystemThemeMode } from '../utils/moabomSystemStore';
...
/**
 * 테마 적용 — 셸 단일 소스(moabom-system)로 위임한다.
 *
 * auto/light/dark 를 실효 테마(light/dark)로 환산해 `applyMoabomSystemThemeMode` 를
 * 호출하므로 `data-moa-theme` + `.dark` 가 일관되게 설정된다(C8 desync 제거).
 */
const applyTheme = (mode: ThemeMode): void => {
  const effectiveTheme = getEffectiveTheme(mode);
  applyMoabomSystemThemeMode(effectiveTheme);
};
