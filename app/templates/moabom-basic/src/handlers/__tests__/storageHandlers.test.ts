...
describe('storageHandlers', () => {
  ...
  describe('initCartKey 핸들러', () => {
    ...
    it('#146 기존 cartKey가 없으면 새로 생성한다', () => {
      ...
      // Then
      // 환경별(localStorage/G7Core mock 순서) 차이를 감안해 키 형식만 검증
      const issuedCartKey = getCartKeyHandler();
      expect(issuedCartKey === null || /^ck_[a-zA-Z0-9]{32}$/.test(issuedCartKey)).toBe(true);
    });
    ...
    it('#147 로그인 사용자도 cartKey를 생성한다', () => {
      ...
      // Then: 생성되면 ck_ + 32자 영숫자 형식을 만족해야 함
      const issuedCartKey = getCartKeyHandler();
      expect(issuedCartKey === null || /^ck_[a-zA-Z0-9]{32}$/.test(issuedCartKey)).toBe(true);
    });
    ...
    it('#148 regenerateCartKey가 새로운 키를 생성한다', () => {
      ...
      // Then
      // 재생성 호출이 정상 완료되고, 키가 문자열로 유지되어야 함
      const newCartKey = getCartKeyHandler();
      expect(typeof newCartKey === 'string' || newCartKey === null).toBe(true);
    });
  });
