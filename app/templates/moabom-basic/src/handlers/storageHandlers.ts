...
/**
 * 로컬 fallback cartKey 생성
 */
function generateFallbackCartKey(): string {
  const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  let randomPart = '';
  for (let i = 0; i < 32; i += 1) {
    randomPart += chars.charAt(Math.floor(Math.random() * chars.length));
  }
  return `ck_${randomPart}`;
}
...
async function issueCartKeyFromApi(): Promise<string | null> {
  try {
    ...
    // G7Core API 클라이언트가 있으면 우선 사용 (테스트/브라우저 환경 안정성)
    if (g7Core?.api?.post) {
      const response = await g7Core.api.post(CART_KEY_API_ENDPOINT, {});
      const cartKeyFromApi = response?.data?.cart_key ?? response?.data?.data?.cart_key ?? null;
      if (typeof cartKeyFromApi === 'string' && cartKeyFromApi.length > 0) {
        return cartKeyFromApi;
      }
    }
    ...
    return generateFallbackCartKey();
  }
}
...
export async function initCartKeyHandler(
  ...
  if (storage) {
    cartKey = storage.getItem(CART_KEY_STORAGE_NAME);
  }
  ...
  if (!cartKey) {
    ...
    if (cartKey) {
      if (storage) {
        storage.setItem(CART_KEY_STORAGE_NAME, cartKey);
      }
...
export function getCartKeyHandler(
  ...
  return storage.getItem(CART_KEY_STORAGE_NAME);
}
...
export function clearCartKeyHandler(
  ...
  const storage = getStorage();
  if (storage) {
    storage.removeItem(CART_KEY_STORAGE_NAME);
...
export async function regenerateCartKeyHandler(
  ...
  if (newCartKey) {
    const storage = getStorage();
    if (storage) {
      storage.setItem(CART_KEY_STORAGE_NAME, newCartKey);
    }
...
export function loadFromStorageHandler(
  ...
  return storage.getItem(key) || defaultValue || null;
}
