...
- 마이페이지 알림 옵션의 설명 문구를 제거하고 시스템 알림 권한 상태를 항목명에 간결하게 표시합니다.
- 중복된 시스템 옵션 데이터가 전달돼도 날씨 효과 토글은 한 번만 표시합니다.

## [0.7.65] - 2026-08-06

### Changed

- 마이페이지 알림 옵션을 알림센터 기록·토스트·시스템 알림·마케팅 동의로 명확히 구분했습니다.
- 시스템 알림의 기기 권한 상태를 함께 표시하고 각 알림 옵션을 독립적으로 선택할 수 있도록 개선했습니다.

## [0.7.64] - 2026-08-06

### Added

- 최초 로그인 사용자가 푸시 권한을 놓치지 않도록 한 번만 표시되는 비차단 안내 토스트와 `알림 켜기` CTA를 추가했습니다.

## [0.7.63] - 2026-08-06

### Added

- 마이페이지 환경설정 하단에 알림센터·배지, 인앱 토스트, 브라우저·모바일 푸시를 구분한 알림 옵션을 추가했습니다.

### Changed

- 브라우저 알림 권한은 로그인 부트가 아닌 사용자의 푸시 옵션 조작에서만 요청합니다.

## [0.7.62] - 2026-08-06

### Fixed

- 실시간 연결이 살아 있어도 이벤트가 멈춘 경우 화면 복귀 시 알림·대화 상태를 복구하고, 숨겨진 탭의 중복 토스트를 방지했습니다.
- private 알림 채널의 실제 수신 확인을 주기적으로 갱신해 FCM fallback이 잘못 생략되지 않도록 수정했습니다.

## [0.7.61] - 2026-08-05

### Added

- 마이페이지 게시글 관리에 앱 리뷰 탭을 추가해 본인이 남긴 리뷰를 함께 확인할 수 있도록 개선했습니다.

### Changed

- 출석 완료 시 버튼을 비활성화하고 다음 자정에 다시 출석할 수 있도록 상태를 동기화했습니다.
- 계정 기본 정보는 비밀번호 확인 후 표시하며, 계정의 휴대폰 번호 문구를 연락처로 통일했습니다.
- 전체검색의 전체앱·마이앱·공개 앱 결과를 마이페이지 라이브러리 카드 형태로 통일했습니다.

### Fixed

- 푸터 이용약관·개인정보처리방침 전용 API가 404일 때 실제 발행 페이지를 대체 조회하도록 수정했습니다.

## [0.7.60] - 2026-08-05

### Changed

- 로그인 부트의 설정 슬라이스가 critical shell-state만 재사용하고 알림·접속 상태는 기존 전용 API로 위임해 생성앱 라이브러리 집계가 포함된 full 요청을 유발하지 않도록 개선했습니다.

## [0.7.59] - 2026-08-05

### Fixed

- 마이페이지 크레딧·활동·프로필이 동일 회원의 인증 또는 번역 상태 갱신을 새 사용자로 오인해 로딩과 내용을 반복 표시하던 문제를 수정했습니다.
- 이미 표시한 크레딧·활동 데이터는 언어 갱신 중 유지하고, Presence 설정도 값이 실제로 바뀔 때만 폼에 동기화하도록 변경했습니다.

## [0.7.58] - 2026-08-05

### Fixed

- 계정 전환 시 열린 창·태스크바와 채팅, 알림, 활동 레벨, 사용자 랭킹, 마이페이지 surface가 이전 사용자 상태를 유지하던 인증 경계 누수를 차단했습니다.
- 랭킹·알림 탭이 첫 페인트에서 빈 상태를 보인 뒤 로딩으로 바뀌거나, 서브탭 변경마다 이미 받은 payload를 다시 조회하던 이중 로딩을 제거했습니다.
- 채팅 inbox 이벤트의 목록 writer를 전역 cache bridge 하나로 통합하고 최초 WebSocket 연결을 재연결로 오인해 구독을 재수립하던 문제를 수정했습니다.

### Changed

- 장애 catch-up API는 접속자 surface가 활성일 때만 Presence 집계를 요청하며, 계정별 앱 커뮤니티·인증 preload 요청도 access scope별로 격리합니다.

## [0.7.57] - 2026-08-05

### Fixed

- 로그인·새로고침 때 접속자 탭의 초기 조회, 로그인 heartbeat 후 재조회, 최초 WebSocket 연결 catch-up이 겹쳐 로딩이 두 번 보이던 문제를 단일 탭 로더로 통합했습니다.
- 계정 전환 중 이전 계정의 Presence 설정·친구 관계 응답이 현재 목록을 덮어쓸 수 있던 인증 스코프 경합을 차단했습니다.
- 로그인 스코프의 첫 접속자 조회가 끝나기 전 빈 상태나 이전 스코프 목록이 한 프레임 노출되지 않도록 초기 조회 상태를 분리했습니다.

### Changed

- 접속자 목록을 플랫폼 summary보다 먼저 조회하고, 활성 탭의 목록만 요청합니다. 정상 WebSocket 최초 연결에는 장애 복구용 통합 상태 API를 실행하지 않으며, 서버가 이미 해석하는 Presence 설정을 heartbeat 앞에서 중복 조회하지 않아 초기 DB·네트워크 병목을 줄였습니다.

## [0.7.56] - 2026-08-05

### Fixed

- 로그인 전후 메인 패널이 이전 계정의 아이콘 배치를 잠시 표시하던 문제를 수정했습니다. 계정별 배치를 분리하고 서버 검증이 끝날 때까지 이전 목록을 노출하지 않습니다.

### Changed

- 로그인 사용자 설정과 생성앱 목록을 공개 셸 부트·인증 확인과 병렬로 선로드해 메인 패널 배치 완료 시간을 줄였습니다.

## [0.7.55] - 2026-08-04

### Changed

- 공개 AI 생성앱 로딩 워터폴 제거: 서버가 실어 보내는 토큰 없는 `preview_url`을 openSeed 로 전달해, Viewer 가 show(메타·권한) 응답을 기다리지 않고 iframe 을 병렬로 즉시 시작합니다. show 는 동일 URL 로 멱등 재확정하므로 재로딩이 없습니다. hosted·비공개 앱은 기존대로 show 의 토큰 포함 URL 을 사용합니다.

## [0.7.54] - 2026-08-04

### Changed

- 네트워크 부트 성능: `shell-boot` SW 전략을 StaleWhileRevalidate 로 바꿔 재방문 시 캐시를 즉시 반환하고 백그라운드로 재검증합니다(콜드 최초 방문만 네트워크 대기).
- 인증 확정이 느려도(콜드 `/api/auth/user`) 앱 카탈로그·후속 로드가 최대 2초만 대기하도록 부트 파이프라인 게이팅을 완화했습니다. 사용자 상태 정확성은 유지됩니다.
- 개별 `window.fetch` 몽키패치(언어 dedupe·user shell-state·shell-boot·shell-critical·ghost-routes)를 단일 인터셉터로 통합해 요청당 URL 파싱을 1회로 줄였습니다.

## [0.7.53] - 2026-08-03

### Changed

- 생성앱·웹사이트 연결 실행 속도: 아이콘 hover/오픈 시 show API 워밍, 웹링크는 카탈로그 URL로 iframe 즉시 시작, AI 앱 로딩 오버레이 soft 대기 600ms→180ms.

## [0.7.52] - 2026-08-03

### Changed

- 네트워크 부트 성능: auth preload 가 이미 친 `/api/auth/user` 상태를 재사용해 실패 시 이중 probe 를 막고, handlers-ready 초기 폴링을 rAF/50ms 로 앞당기며, pureRouter shell-boot 전략을 SW NetworkFirst 와 맞춥니다.

## [0.7.51] - 2026-08-03

### Fixed

- 직접 입력 HTML(`html_paste`) 앱을 좌하단 앱편집으로 열면 미리보기에 요소선택 버튼이 안 뜨던 문제 — 생성앱과 같이 Inspector 패널·패치 스트림을 허용합니다.

## [0.7.50] - 2026-07-30

### Fixed

- 우측 패널 접속자 탭: 탭 전환 시 새로고침이 친구/알림과 달리 빠져 있던 문제 — `useShellSubTabSelect`가 탭 이동 시에도 onSelect(갱신)를 호출하도록 수정.

## [0.7.49] - 2026-07-29

### Changed

- AI 앱 만들기 모델 선택: 코딩 특화 6칸(Claude Sonnet/Haiku · GPT-5.4/Mini · Gemini 3.6 Flash / 3.5 Flash-Lite).
- AI 스마트챗 모델 라벨을 API `label`로 표시(대화 특화 6칸).

## [0.7.48] - 2026-07-29

### Changed

- AI 스마트챗: 「관련 앱 만들기」 유지, 분기 버튼·배너를 「여기서부터 다시 묻기」 / 「이 답변 이후는 새 갈래로 이어집니다」 / 「갈래 취소」로 변경.

## [0.7.47] - 2026-07-29

### Fixed

- AI 스마트챗 → AI 앱 만들기 핸드오프: create-app 청크가 프롬프트 세션을 복제해 비어 보이던 문제 — `window.__MoabomCreateAppPrompt` 싱글톤을 edit 세션과 같이 external 공유합니다. 제목·프롬프트를 함께 전달해 폼에 자동 입력합니다.

### Changed

- 웹검색 토글: 켜짐=`primary-outline`+check, 꺼짐=`dark-outline`+globe.

## [0.7.46] - 2026-07-29

### Fixed

- 접속자: 로그인 후 본인 IP 방문자 shadow 가 남던 문제 — login 시 목록·summary 재조회를 계약대로 강제하고, 동일 마스크 IP guest 를 클라에서 숨깁니다. heartbeat/폴링 주기는 변경하지 않습니다.

### Changed

- AI 스마트챗 메시지 액션(복사·분기·앱 만들기·기억하기)과 웹 검색 토글을 `dark-outline` 으로 통일. 웹 검색 켜짐 시 아이콘을 globe→check 로 바꿉니다.

## [0.7.45] - 2026-07-28

### Changed

- AI 스마트챗 "이 답으로 앱 만들기" — 답변 원문 대신 질문+답변을 요약한 앱 제작 프롬프트를 생성해 AI 앱 만들기에 채워 넣습니다. 생성 중 버튼 스피너("앱 프롬프트 작성 중…") 표시, 실패 시 질문+답변 구조화 폴백.
- AI 스마트챗 "기억하기" — 답변을 500자에서 자르던 것 대신 전문을 서버로 보내 핵심 팩트만 요약해 저장합니다.
- AI 스마트챗 대화설정 — 열리면 대화 영역을 짓누르는 대신 메시지 영역과 같은 표면(glass-sm-blur)으로 대체하고, 툴바 버튼이 닫기 토글(danger-outline + X 아이콘)로 바뀝니다. 설정이 열린 동안 입력창·찌그러진 잔여 대화창을 숨기고 닫으면 스크롤을 복원합니다.
- AI 스마트챗 대화 내보내기 — md 대신 일반 이용자에게 익숙한 txt 파일로 내보냅니다.
- AI 스마트챗 대화공유 — 버튼에 진행 스피너·완료 피드백을 추가하고, 공유 생성 시 링크를 자동으로 클립보드에 복사합니다("복사됨" 표시). 링크 행의 복사 버튼도 복사 완료를 표시합니다.

## [0.7.44] - 2026-07-27

### Removed

- 회의용 셸 앱 `mysum-integration`(마이숨 데이터 연동) 및 관련 스타일·i18n. 브리프 내용은 워크스페이스 루트 PPT로 아카이브.

## [0.7.43] - 2026-07-27

### Changed

- 생성앱 실행(뷰어)은 show를 `include_html=0` 메타만 받고 iframe은 preview_url로 HTML을 로드합니다. 편집·리믹스는 HTML 포함 show를 유지합니다.
- 생성앱 iframe은 onLoad 후 soft ready(600ms)로 로딩 오버레이를 더 빨리 해제합니다.

## [0.7.42] - 2026-07-13

### Fixed

- 로그인 사용자 UUID가 늦게 복원돼도 알림 private 채널을 재구독하고, FCM 수신 시 우측 알림 패널의 목록·미읽음 수를 서버 상태와 동기화.
- FCM 알림 클릭은 전체 화면 이동 대신 기존 셸 앱 윈도우로 연결.

## [0.7.41] - 2026-07-12

### Changed

- Presence: 로그인 후 revision WS 구독은 유지하고, 접속자/친구 목록 refetch·주기 heartbeat 만 탭 활성 시에 실행해 실시간 체감을 복구.
- 프로필 「친구해제」 버튼을 danger(빨강) 대신 primary(포인트 컬러)로 변경.

## [0.7.39] - 2026-07-12

### Fixed

- Presence: 로그인 heartbeat 직후 접속자 목록을 다시 조회하고, 동일 마스크 IP guest 잔여 행을 낙관적으로 제거해 로그인 직후 「나 + 방문자」 중복이 남지 않게 함.

## [0.7.38] - 2026-07-12

### Added

- FCM 디바이스 토큰 등록 (`moabomFcmClient`) — 로그인·알림 권한 후 `moabom-fcm` API 연동
...
- 메시지 UI 전역 명칭 정리 — 「대화/대화거부」→「메시지/차단」, 차단 상태 버튼 「차단해제」, API·토스트·마이페이지 접속 설정 문구 포함 (ko/en/ja/zh).
