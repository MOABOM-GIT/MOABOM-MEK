...
import type { ComponentDefinition } from '../../../../../resources/js/core/template-engine/DynamicRenderer';

function comp(node: ComponentDefinition | string | undefined): ComponentDefinition {
...
function child(component: ComponentDefinition, index: number): ComponentDefinition {
  return comp(component.children?.[index]);
}
...
describe('템플릿 업데이트 - 업데이트 모달', () => {
  ...
  describe('모달 콘텐츠 구조 검증', () => {
    ...
    it('모달에 업데이트 확인 메시지가 포함되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const confirmP = child(child(modal, 0), 0);
    ...
    it('모달에 버전 정보가 포함되어 있다 (from → to)', () => {
      const modal = updateModalLayout.modals![0];
      const versionRow = child(child(child(modal, 0), 1), 0);
      const versionLabel = child(versionRow, 0);
      const versionValue = child(versionRow, 1);
    ...
    it('모달에 update_source 표시가 포함되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const vendorRow = child(child(child(modal, 0), 1), 1);
      const vendorLabel = child(vendorRow, 0);
      const vendorValue = child(vendorRow, 1);
  ...
  describe('레이아웃 전략 선택 UI', () => {
    it('레이아웃 전략 섹션이 모달에 포함되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const strategySection = child(child(modal, 0), 3);
      expect(child(strategySection, 0).text).toBe('$t:admin.templates.modals.layout_strategy_title');
    });
    ...
    it('수정된 레이아웃이 없을 때 녹색 메시지가 표시된다 (조건: !hasModifiedLayouts)', () => {
      ...
      const warningDiv = child(strategySection, 1);
      ...
      const noModifiedDiv = child(strategySection, 2);
    ...
    it('overwrite 라디오 옵션이 올바르게 구성되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const strategySection = child(child(modal, 0), 3);
      const radioGroup = child(strategySection, 3);
      const overwriteLabel = child(radioGroup, 0);
      const overwriteInput = child(overwriteLabel, 0);
      const overwriteText = child(overwriteLabel, 1);
      ...
      expect(child(overwriteText, 0).text).toBe('$t:admin.templates.modals.layout_strategy_overwrite');
      expect(child(overwriteText, 1).text).toBe('$t:admin.templates.modals.layout_strategy_overwrite_desc');
    });
    ...
    it('keep 라디오 옵션이 올바르게 구성되어 있다', () => {
      ...
      const strategySection = child(child(modal, 0), 3);
      const radioGroup = child(strategySection, 3);
      const keepLabel = child(radioGroup, 1);
      const keepInput = child(keepLabel, 0);
      const keepText = child(keepLabel, 1);
      ...
      expect(child(keepText, 0).text).toBe('$t:admin.templates.modals.layout_strategy_keep');
      expect(child(keepText, 1).text).toBe('$t:admin.templates.modals.layout_strategy_keep_desc');
    });
  ...
  describe('버튼 구조 및 상태 검증', () => {
    it('취소/확인 버튼이 모달에 정의되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const buttonsDiv = child(modal, 1);
      expect(buttonsDiv.children).toHaveLength(2);

      const cancelBtn = child(buttonsDiv, 0);
      const confirmBtn = child(buttonsDiv, 1);

      expect(cancelBtn.name).toBe('Button');
      expect(cancelBtn.props?.disabled).toBe('{{_global.isTemplateUpdating}}');
      expect(child(cancelBtn, 0).text).toBe('$t:common.cancel');
    ...
    it('확인 버튼에 스피너 조건부 렌더링이 설정되어 있다', () => {
      const modal = updateModalLayout.modals![0];
      const confirmBtn = child(child(modal, 1), 1);

      const spinnerIcon = child(confirmBtn, 0);
      ...
      const textSpan = child(confirmBtn, 1);
