...
- `template.json`의 `g7_version`을 `>=7.0.0-beta.1,<8.0.0`로 조정(코어 빠른 베타·7.x 정식 호환, 8.x 메이저는 명시적 포팅 전까지 제외)

## [1.0.0-beta.12] - 2026-05-12

### Added

- **`sirsoft-tosspayments` lazy 대비**: `src/runtime/moabomAdminDeferredPluginPrefetch.ts` — `sirsoft-tosspayments`가 포함된 관리자 레이아웃 fetch 전 `loadDeferredExtensionAssets` 선로딩. Vitest `moabomAdminDeferredPluginPrefetch.test.ts`.

## [1.0.0-beta.11] - 2026-05-06

### Changed

- **HomeBackgroundManager 포인트 컬러 드롭다운을 색상 스와치 팝오버로 교체** — admin `Select` 가 썸네일 카드 폭에 갇혀 옵션 라벨이 줄바꿈되던 UI 깨짐 해결. 썸네일 아래 버튼 클릭 시 팝오버가 열리고 **3 열 동그라미 스와치 그리드** + 최상단 "없음" 버튼으로 구성. 선택값은 스와치 동그라미(또는 "없음") 텍스트로만 표시해 폭이 좁아도 깔끔하게 보인다. 배경 저장 포맷(`mode` / `point_color`) 은 그대로 유지.

## [1.0.0-beta.10] - 2026-05-06

### Changed

- **HomeBackgroundManager 포인트 컬러 드롭다운 라벨 간소화** — 썸네일 카드 폭이 좁아 긴 라벨이 세로로 줄바꿈되던 문제 해결. 옵션 라벨을 "포인트 컬러 없음 → 없음", "1. #6366f1 → #6366f1" 로 단순화 (`HomeBackgroundManager.tsx`).
- `pointColorPresets` prop 타입 방어 강화 — 배열이 아닌 값(문자열/undefined)이 들어오면 빈 배열로 정규화. 레이아웃에서는 `computed` 로 배열 확정 후 전달하는 것이 안전(`admin_mypage_settings.json` 의 `pointColorPresetsSafe` 참고).

## [1.0.0-beta.9] - 2026-05-06

### Added

- **`HomeBackgroundManager` 에 모드·포인트 컬러 바인딩** — 각 업로드 배경 썸네일 아래 드롭다운 두 개 추가 (`HomeBackgroundManager.tsx`):
  - `mode` 드롭다운: `라이트 모드` / `다크 모드` — 사용자 마이페이지가 현재 테마에 맞춰 해당 모드 배경만 노출하는 근거가 된다.
  - `point_color` 드롭다운: `포인트 컬러 없음` 또는 관리자 팔레트 각 hex — 사용자가 마이페이지 팔레트에서 그 색을 선택하면 이 배경이 자동으로 같이 선택된다.
- **포인트 컬러 배타 바인딩** — 같은 hex 를 다른 배경에 지정하면 기존 배경의 `point_color` 를 자동으로 `null` 로 해제한다 (프론트). 서버 저장 시에도 `SystemSettingsService::stripAppearanceForStorage` 가 유일성을 한 번 더 강제한다.
- 새 prop `pointColorPresets: string[]` — 상위 레이아웃(`admin_mypage_settings.json`)에서 현재 `point_color_presets` 를 주입한다.

### Changed

- `HomeBackgroundItem` 타입에 `mode?: 'light' | 'dark'` / `point_color?: string | null` 필드 추가.

## [1.0.0-beta.8] - 2026-05-05

### Added

- **SortableMypageMenuList**, **HomeBackgroundManager** 컴포지트 — moabom-system 마이페이지 설정 레이아웃에서 메뉴 드래그 편집·홈 배경 업로드 UI에 사용

## [1.0.0-beta.6] - 2026-05-04

### Fixed

- IIFE 번들 전역 변수명을 `MoabomAdminBasic`으로 수정 — 식별자 `moabom-admin_basic`에 맞춰 `ComponentRegistry`와 일치 (`vite.config.ts`)

## [1.0.0-beta.5] - 2026-05-04

### Changed

- 템플릿 식별자를 `moabom-admin_basic`으로 분리 — 업스트림 원본(`sirsoft-admin_basic`)과 포크 관리 명확화 (`template.json`, `components.json`, 패키지명, 로거·문서 문자열)

## [1.0.0-beta.4] - 2026-05-04

### Fixed

- 템플릿 커스텀 `setLocale` 핸들러가 엔진 빌트인을 덮어 관리자 UI에서 `TemplateApp.changeLocale`이 동작하지 않던 문제 수정 — 빌트인 사용으로 통일
- 환경설정 > 일반 > 지역화에서 기본 언어 저장 후 관리자 화면 로케일이 갱신되지 않던 문제 — 저장 성공 시 `setLocale` 호출 추가
