...
LEGACY_SLUG="${SAAS_LEGACY_APPEARANCE_SLUG:-autocloud864134}"
...
if [[ -n "${LEGACY_SLUG}" ]]; then
  echo "[3/3] legacy appearance re-apply slug=${LEGACY_SLUG}"
  ...
  echo "[3/3] skip legacy appearance (SAAS_LEGACY_APPEARANCE_SLUG empty)"
fi
